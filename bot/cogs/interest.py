...
class Interest(commands.Cog):
    def __init__(self, bot):
        ...
        if not self.cog:
            print(
                "⚠️  Interest cog: Economy cog not found. Interest system disabled."
            )
            return
                ...
                async with conn.cursor() as cur:
                    ...
                    await cur.execute("""
                        SELECT
                            ...
                            COUNT(p.property_type) as property_count
                        FROM users u
                        ...
                        try:
                            ...
                            if user:
                                embed = discord.Embed(
                                    ...
                                    description=(
                                        f"Vous avez reçu **{self.cog._fmt_amount(interest_amount)}** "
                                        "d'intérêts sur vos immeubles !"
                                    ),
                                    color=discord.Color.gold(),
                                    timestamp=datetime.now()
                                )
                                embed.add_field(
                                    name="Immeubles possédés",
                                    value=f"{property_count}",
                                    inline=True,
                                )
                                embed.add_field(
                                    name="Revenus totaux",
                                    value=f"{self.cog._fmt_amount(total_income)}/h",
                                    inline=True,
                                )
                                embed.add_field(name="Taux d'intérêt", value="10%", inline=True)
                                embed.set_footer(
                                    text="Les intérêts sont versés automatiquement tous les jours"
                                )
