...
class Economy(commands.Cog):
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if flip == side:
                    ...
                    color = discord.Color.red()
        emb = self._bank_embed(ctx, title="Coin Flip", description=desc, color=color, txn_id=txid) if txid else discord.Embed(description=desc, color=color)
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if win > 0:
                    ...
                    color = discord.Color.red()
        emb = self._bank_embed(ctx, title="Slots", description=desc, color=color, txn_id=txid) if txid else discord.Embed(description=desc, color=color)
        view = TaxTransactionView(self, txid) if txid else None
        await ctx.send(embed=emb, view=view)
    ...
    async def dice(self, ctx: commands.Context, amount: int | None = None, bet_on: int | None = None):
        ...
        emb = self._bank_embed(ctx, title="Dé", description=desc, color=color, txn_id=txid) if txid else discord.Embed(description=desc, color=color)
        view = TaxTransactionView(self, txid) if txid else None
        await ctx.send(embed=emb, view=view)
...
class CoinFlipView(discord.ui.View):
    ...
    async def play(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not self.side:
            return await interaction.response.send_message("Choisis Pile ou Face.", ephemeral=True)
        ctx = self.ctx
        ...
        async with self.cog.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if a_bal < self.amount or t_bal < self.amount:
                    return await interaction.response.send_message("Solde insuffisant pour l’un des deux.", ephemeral=True)
        ...
        async with self.cog.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if win > 0:
                    ...
                    color = discord.Color.red()
        emb = self.cog._bank_embed(self.ctx, title="Slots", description=desc, color=color, txn_id=txid) if txid else discord.Embed(description=desc, color=color)
        view = TaxTransactionView(self.cog, txid) if txid else None
        await interaction.response.edit_message(embed=emb, view=view)
        ...
        async with self.cog.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if win >= 0:
                    ...
                    color = discord.Color.red()
        emb = self.cog._bank_embed(self.ctx, title="Dé", description=desc, color=color, txn_id=txid) if txid else discord.Embed(description=desc, color=color)
        view = TaxTransactionView(self.cog, txid) if txid else None
        await interaction.response.edit_message(embed=emb, view=view)
    ...
    async def tax_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with self.cog.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                row = await cur.fetchone()
                if not row:
                    return await interaction.response.send_message("Transaction introuvable.", ephemeral=True)
                target_id, amount, col, status, taxed = row
                if status not in ("accepted", "won"):
                    return await interaction.response.send_message("Transaction non taxable.", ephemeral=True)
                if taxed:
                    return await interaction.response.send_message("Taxe déjà appliquée.", ephemeral=True)
                tax_amt = max(1, int(amount * TAX_RATE))
                await cur.execute(f"SELECT {col} FROM users WHERE user_id=%s", (target_id,))
                avail = (await cur.fetchone())[0]
                tax_real = min(tax_amt, avail)
                if tax_real <= 0:
                    return await interaction.response.send_message("Rien à taxer.", ephemeral=True)
                await cur.execute(f"UPDATE users SET {col}={col}-%s WHERE user_id=%s", (tax_real, target_id))
...
class AdminTransactionView(discord.ui.View):
    ...
    async def accept_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with self.cog.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                target_id, amount, col, status = row
                if status != "pending":
                    return await interaction.response.send_message("Déjà traitée.", ephemeral=True)
                await cur.execute(f"UPDATE users SET {col}={col}+%s WHERE user_id=%s", (amount, target_id))
                await cur.execute("UPDATE transactions SET status='accepted' WHERE id=%s", (txid,))
        await interaction.response.send_message("Accepté.", ephemeral=True)
    ...
    @commands.command(name="entreprises", help="Lister entreprises et prix")
    ...
    @discord.ui.select(placeholder="Parie un nombre", options=[discord.SelectOption(label=str(i), value=str(i)) for i in range(1,7)])
...
async def setup(bot: commands.Bot):
    ...
    async def scoot_slash(interaction: discord.Interaction, membre: discord.Member, montant: int):
        ...
        if interaction.user.id == membre.id:
            return await interaction.response.send_message("Choisis quelqu’un d’autre.", ephemeral=True)
        if montant <= 0:
    ...
    async def slots_slash(interaction: discord.Interaction, amount: int):
        ...
        color = discord.Color.green() if win > 0 else discord.Color.red()
        emb = cog._bank_embed(ctx, title="Slots", description=desc, color=color, txn_id=txid)
        await interaction.response.send_message(embed=emb)
    ...
    async def dice_slash(interaction: discord.Interaction, amount: int, bet_on: int | None = None):
        ...
        if amount <= 0:
            return await interaction.response.send_message("Montant invalide.", ephemeral=True)
        if bet_on is not None and not (1 <= bet_on <= 6):
            return await interaction.response.send_message("Parie sur un nombre entre 1 et 6.", ephemeral=True)
        import random
        txid = None
        async with cog.pool.acquire() as conn:
            async with conn.cursor() as cur:
        ...
        color = discord.Color.green() if win > 0 else discord.Color.red()
        emb = cog._bank_embed(ctx, title="Dé", description=desc, color=color, txn_id=txid)
        await interaction.response.send_message(embed=emb)
    ...
    async def entreprise_slash(interaction: discord.Interaction, name: str):
        ...
        async with cog.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                bank = (await cur.fetchone())[0]
                if bank < 100000:
                    return await interaction.response.send_message("Il faut 100000 en banque.", ephemeral=True)
                await cur.execute("SELECT id FROM enterprises WHERE name=%s", (name,))
                if await cur.fetchone():
                    return await interaction.response.send_message("Nom déjà pris.", ephemeral=True)
